...
public interface GiayChiTietRepository
    ...
    List<GiayChiTietEntity> findByGiayEntityIdAndMauSacEntityId(@Param("giayId") UUID giayId,
                                                                @Param("mauSacId") UUID mauSacId);

    // Tìm theo ID thương hiệu và ID màu sắc
    @Query("SELECT g FROM GiayChiTietEntity g WHERE g.giayEntity.thuongHieu.id = :thuongHieuId AND g.mauSacEntity.id = :mauSacId")
    List<GiayChiTietEntity> findByGiayEntityThuongHieuIdAndMauSacEntityId(@Param("thuongHieuId") UUID thuongHieuId,
                                                                          @Param("mauSacId") UUID mauSacId);
